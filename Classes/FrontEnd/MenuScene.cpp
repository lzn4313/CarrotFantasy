...
void MenuScene::initCloud()
{
    ...
    cloud1->setPosition(Vec2(cloud1->getContentSize().width, visibleSize.height * 0.85));
    cloud1->setOpacity(230);
    //添加动画
    //从左向右飘
    auto cloud1_moveto_1 = MoveTo::create(20, Vec2(visibleSize.width + cloud1->getContentSize().width, visibleSize.height * 0.85));
    //从右向左飘
    auto cloud1_moveto_2 = MoveTo::create(20, Vec2(cloud1->getContentSize().width, visibleSize.height * 0.85));
    ...
    cloud2->setPosition(Vec2(visibleSize.width + cloud2->getContentSize().width,
        visibleSize.height * 0.8));
    cloud2->setScale(2);
    cloud2->setOpacity(230);
    //添加动画
    //从右向左
    auto cloud2_moveto_1 = MoveTo::create(20, Vec2(cloud2->getContentSize().width, visibleSize.height * 0.85));
    //从左向右
    auto cloud2_moveto_2 = MoveTo::create(20, Vec2(visibleSize.width + cloud2->getContentSize().width, visibleSize.height * 0.85));
...
void MenuScene::initMonster()
{
    ...
    monster->setPosition(Vec2(visibleSize.width / 4,visibleSize.height * 0.8));
...
void MenuScene::initCarrot()
{
   ...
   carrot->setPosition(Vec2(visibleSize.width / 2,visibleSize.height / 4));
   this->addChild(carrot);
   //1,添加萝卜叶子
   auto carrot_leaf1 = Sprite::createWithTexture(Director::getInstance()->getTextureCache()->getTextureForKey("MenuScene/Leaf1.PNG"));
   carrot_leaf1->setPosition(Vec2(-carrot_leaf1->getContentSize().width / 2, visibleSize.height / 4 + carrot_leaf1->getContentSize().height * 1.2));
   ...
   carrot_leaf2->setPosition(Vec2(carrot_leaf2->getContentSize().width * 0.6, visibleSize.height / 4 + carrot_leaf2->getContentSize().height * 1.3));
   ...
   carrot_leaf3->setPosition(Vec2(0,visibleSize.height / 4 + carrot_leaf3->getContentSize().height * 1.1));
   ...
   carrot_body->setPosition(Vec2(0, visibleSize.height / 4 + carrot_body->getContentSize().height / 4));
...
void MenuScene::initTitle()
{
    ...
    title->setPosition(Vec2(visibleSize.width / 2 + title->getContentSize().width / 20, visibleSize.height / 2 - title->getContentSize().height / 7));
    this->addChild(title);
}
//初始化菜单按钮
void MenuScene::initMenu()
{
    ...
    close_item->setPosition(Vec2(visibleSize.width - close_item->getContentSize().width * 0.6, visibleSize.height - close_item->getContentSize().height / 2));
    ...
    options_item->setPosition(Vec2(visibleSize.width * 0.2, visibleSize.height / 3));
    ...
    helper_item->setPosition(Vec2(visibleSize.width * 0.8, visibleSize.height / 3));
    ...
    adventure_item->setPosition(Vec2(visibleSize.width / 2 - adventure_item->getContentSize().width, visibleSize.height / 8));
    ...
    boss_item->setPosition(Vec2(visibleSize.width / 2, visibleSize.height / 8));
    ...
    if (UserDefault::getInstance()->getIntegerForKey("if_boss_lock") == 1) {//若boss模式未解锁，则生成一个“锁”的图形
        ...
        boss_lock->setPosition(Vec2(visibleSize.width / 2 + boss_item->getContentSize().width * 0.41, visibleSize.height / 8 - boss_item->getContentSize().height / 5));
    ...
    nest_item->setPosition(Vec2(visibleSize.width / 2 + nest_item->getContentSize().width,visibleSize.height / 8));
    ...
    if (UserDefault::getInstance()->getIntegerForKey("if_nest_lock") == 1) {//若怪物窝未解锁，则生成一个“锁”的图形
        ...
        nest_lock->setPosition(Vec2(visibleSize.width / 2 + nest_item->getContentSize().width * 1.44, visibleSize.height / 8 - nest_item->getContentSize().height / 5));
